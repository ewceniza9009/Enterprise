...
namespace FMISLicenseMgr
{
    public class License
    {
        ...
        private int GetCurrentUserId(Access.Application FMIS)
        {
            ...
            Access.Dao.Recordset rs = currDB.OpenRecordset("SELECT UserId FROM SysCurrent", Access.Dao.RecordsetTypeEnum.dbOpenDynaset, Access.Dao.RecordsetOptionEnum.dbSeeChanges);

            if (rs.RecordCount > 0)
            {
                rs.MoveFirst();
                DefaultUserId = rs.Fields["UserId"].Value;
            }
        ...
        public bool OpenForm(Access.Application FMIS, Access.Form FrmActiveForm, int UserId, bool CheckRecordLocked)
        {
            ...
            try
            {
                ...
                else if (licenseCode.ToUpper() == "TRIAL")
                {
                    ...
                    {
                        MessageBox.Show("Bad date, Please contact our support team to resolve this issue", "License", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return false;
                    }
                    else
                    {
                        licensedSoftware = false;
                        MessageBox.Show("Trial period expired, Please contact our support team to resolve this issue", "License", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                ...
                {
                    MessageBox.Show("No License.", "Security", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        public void LockForm(Access.Application FMIS, Access.Form FrmActiveForm, bool BolLocked, int UserId, bool CheckRecordLocked)
        {
            ...
            if (FrmActiveForm.Name.Substring(0, 3).ToUpper() == "REP")
            {
                ...
                catch
                {
                    ...
                    {
                        if (control.Name == "Image36")
                        {
                            Access.Image Image36 = (Access.Image)control;
                            Image36.Picture = appDataPath + "\\etechLogo.png";
            ...
            else
            {
                foreach (Access.Control objControl in FrmActiveForm.Controls)
                {
                    if (objControl.Name.Substring(((objControl.Name.Length) - 3), 3) == "Mem")
                    {
        ...
        public void Trial()
        {
            try
            {
                ...
                string txtPath = Path.Combine(fullFolderPath, "trial.lic");
        ...
        public string GetLicenseCode()
        {
            ...
            string appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            string fullFolderPath = Path.Combine(appDataPath, "eTech");
            string txtPathLicensed = Path.Combine(fullFolderPath, "license.lic");
        ...
        public void SaveLicenseCode(string license)
        {
            string appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            string fullFolderPath = Path.Combine(appDataPath, "eTech");
            string txtPath = Path.Combine(fullFolderPath, "license.lic");
        ...
        public void UnlicenseSoftware()
        {
            ...
            string txtPathTrial = Path.Combine(fullFolderPath, "trial.lic");
            string txtPathLicense = Path.Combine(fullFolderPath, "license.lic");
